...
(() => {
  ...
  const getSource = () => window.archiveLoadMoreSource || btn.dataset.source;
  const getPage = () => Number(btn.dataset.page || "2");
  ...
  const resolveTargetRow = () => {
    ...
    return rows.length ? rows[rows.length - 1] : grid;
  };

  const appendFromResponse = (html) => {
    const fragment = parseFragment(html);
    const cols = Array.from(fragment.querySelectorAll('[class*="col-"]'));
    if (cols.length) {
  ...
  btn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      const appended = appendFromResponse(data.html);
      if (!appended) {
        hideButton();
        return;
      }
